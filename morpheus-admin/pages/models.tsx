...
export default function Home() {
  ...
  useEffect(() => {
    getAvailableModels()
      .then((response: Response) => {
        ...
        all_models.sort((a, b) => (a.is_active === b.is_active ? 0 : a.is_active ? -1 : 1));
  ...
  useEffect(() => {
    ...
    const modelsFiltered = models.filter((model) => {
      const statusFilter = filters.is_active === "all" || model.is_active === (filters.is_active === "active");

      const kindFilter = filters.kind === "all" || model.kind === filters.kind;

      const nameFilter = model.name.toLowerCase().includes(filters.name.toLowerCase());

      const sourceFilter = model.source.toLowerCase().includes(filters.source.toLowerCase());

      return statusFilter && kindFilter && nameFilter && sourceFilter;
    });

    modelsFiltered.sort((a, b) => a.name.localeCompare(b.name));

    setFilteredModels(modelsFiltered);
  }, [models, filters]);

  const filterStatusOptions: Array<string> = ["all", "active", "inactive"];
  const filterKindOptions: Array<string> = ["all", ...new Set(models.map((model: Model) => model.kind))];
          ...
          {models.length > 0 && (
            <div className="overflow-x-auto">
              <div className="flex w-3/4 my-5 space-x-4 ">
                ...
                />
                <Select name="kind" label="filter by kind" options={filterKindOptions} onChange={handleFilterChange} />
                <TextInput
                  name="name"
              ...
              {editingModel && (
                <Modal
                  ...
                  onClose={() => {
                    if (!loading) setEditingModel(null);
                  }}
                >
                  <ModelForm
                    title={"Edit Model"}
              ...
              {removingModel && (
                <Modal
                  ...
                  onClose={() => {
                    if (!loading) setRemovingModel(null);
                  }}
                >
                  <div className="modal-body">
                    ...
                    <div className="flex flex-row justify-center gap-8 pt-4">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          if (!loading) handleRemoveModel(removingModel);
                        }}
                      />
                    </div>
                  </div>
