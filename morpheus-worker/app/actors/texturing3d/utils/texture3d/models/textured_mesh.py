...
from app.utils.texture3d.configs.train_config import GuideConfig
from app.utils.texture3d.models.mesh import Mesh
from app.utils.texture3d.models.render import Renderer
...
class TexturedMeshModel(nn.Module):
    def __init__(
        ...
        self.mesh.normalize_mesh(inplace=True, target_scale=self.mesh_scale, dy=self.dy)
    ...
    def init_meshes(self, env_sphere_path="./mnt/shapes/env_sphere.obj"):
        if not Path(env_sphere_path).exists():
            model_url = "https://raw.githubusercontent.com/TEXTurePaper/TEXTurePaper/main/shapes/env_sphere.obj"
            response = requests.get(model_url)
            if response.status_code == 200:
                os.makedirs(Path(env_sphere_path).parent, exist_ok=True)
                with open(env_sphere_path, "wb") as f:
    ...
    def init_texture_map(self):
        ...
        if (
            ...
            run_xatlas = not (ft.shape[0] == self.mesh.faces.shape[0])
        else:
            run_xatlas = True
        ...
        if run_xatlas:
            ...
            print("-----------------------------------------------------------")
            print(self.mesh.vertices.shape)
            print(self.mesh.faces.shape)
            print("-----------------------------------------------------------")
    ...
    @torch.no_grad()
    def export_mesh(self, path, save_name=None):
        ...
        texture_file = os.path.join(path, f"{name}{save_name}.png")
        colors.save(texture_file)

        # save obj (v, vt, f /)
        obj_file = os.path.join(path, f"{name}{save_name}.obj")
        mtl_file = os.path.join(path, f"{name}{save_name}.mtl")

        logger.info("writing obj mesh to {obj_file}")
        with open(obj_file, "w") as fp:
            fp.write(f"mtllib {name}{save_name}.mtl \n")
        ...
        with open(mtl_file, "w") as fp:
            ...
            fp.write(f"map_Kd {name}{save_name}.png \n")
