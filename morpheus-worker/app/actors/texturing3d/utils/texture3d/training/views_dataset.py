...
from app.utils.texture3d.configs.train_config import RenderConfig
from app.utils.texture3d.utils import get_view_direction
...
class MultiviewDataset:
    def __init__(self, cfg: RenderConfig, device):
        ...
        if hasattr(cfg, "radius_per_view"):
            self.multiple_radius = True
        else:
            self.multiple_radius = False

        # view angles are equidistant
        if self.cfg.view_angles is None:
        ...
        # view angles can be defined manually instead
        else:
            self.phis = self.cfg.view_angles
            size = len(self.phis)
            self.thetas = [self.cfg.base_theta for _ in range(size)]

        # append overhead and bottom views
    ...
    def collate(self, index):
        ...
        if self.multiple_radius:
            radius = self.cfg.radius_per_view[index[0]]
        else:
...
class ViewsDataset:
    def __init__(self, cfg: RenderConfig, device, size=100, base_radius=1.75):
        super().__init__()
        ...
        if hasattr(cfg, "radius_per_view"):
            self.radius = max(cfg.radius_per_view)
        else:
