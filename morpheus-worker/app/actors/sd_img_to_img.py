...
class StableDiffusionImageToImage(StableDiffusionAbstract):
    ...
    def _warmup(self):
        if self.device != "cuda":
            return
        self.logger.info("Running img2img warmup pass...")
        try:
            dummy = Image.new("RGB", (128, 128))
            with torch.no_grad():
                self.pipeline(image=dummy, prompt="warmup", num_inference_steps=1, output_type="latent", strength=0.99)
            self.logger.info("Warmup complete")
        except Exception as e:
            self.logger.warning(f"Warmup failed (non-fatal): {e}")

    def generate(self, request: ModelRequest):
        self.logger.info(f"StableDiffusionImageToImage.generate: request: {request.dict(exclude={'image', 'palette_image', 'mask'})}")
