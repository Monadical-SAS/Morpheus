...
DEFAULT_UPSCALING_MODEL_PATH = (
    f"{settings.model_parent_path}{settings.upscaling_model_default}"
)
...
@check_environment
def generate_stable_diffusion_text2img_output_task(
    ...
    try:
        ...
        logger.info(
            f"Current model: {self.model.model_name} - Model selected: {model_selected}"
        )
        logger.info(
            f"Current scheduler: {self.model.scheduler} - Sampler selected: {scheduler_selected}"
        )
...
@check_environment
def generate_stable_diffusion_xl_text2img_output_task(
    self, request: GenerationRequest
) -> list[str]:
    try:
        model_selected = request.model_id
        ...
        if request.color_palette == "None":
            ...
            if request.color_palette == "Quantization - Blend" and palette_image:
                ...
                quantized_image = recoloring.quantize(
                    image, palette=color_list, use_gray_level=False
                )
                ...
                color_list = recoloring.get_image_colors(palette_image)
                quantized_image = recoloring.quantize(
                    image, palette=color_list, use_gray_level=True
                )
                conditioning_image = Image.blend(quantized_image, image, alpha=0.3)

            elif (
                request.color_palette == "Quantization Gray - Contours"
                and palette_image
            ):
                color_list = recoloring.get_image_colors(palette_image)
                quantized_image = recoloring.quantize(
                ...
                polygon_image = polygon_image.filter(
                    ImageFilter.GaussianBlur(radius=20)
                )
                conditioning_image = Image.blend(polygon_image, image, alpha=0.15)

            elif (
                request.color_palette == "Random Polygons - Contours" and palette_image
            ):
                color_list = recoloring.get_image_colors(palette_image)
                polygon_image = recoloring.random_polygon_image(
                    color_list, width=image.width, height=image.height, num_polygons=150
                ...
                contours = generate_canny_image(image)
                conditioning_image = recoloring.draw_contours_on_image(
                    contours, polygon_image
                )

            elif (
                request.color_palette == "Random Color Blocks Small - Blend"
                and palette_image
            ):
                color_list = recoloring.get_image_colors(palette_image)
                random_block_image = recoloring.create_block_image(
                    8, 8, image.width, image.height, color_list
                )
                conditioning_image = Image.blend(random_block_image, image, alpha=0.15)

            elif (
                request.color_palette == "Random Color Blocks Small - Contours"
                and palette_image
            ):
                color_list = recoloring.get_image_colors(palette_image)
                random_block_image = recoloring.create_block_image(
                ...
                request.color_palette == "Random Color Blocks Large - Blend"
                and palette_image
            ):
                color_list = recoloring.get_image_colors(palette_image)
                random_block_image = recoloring.create_block_image(
                    3, 3, image.width, image.height, color_list
                )
                conditioning_image = Image.blend(random_block_image, image, alpha=0.3)

            elif (
                request.color_palette == "Random Color Blocks Large - Contours"
                and palette_image
            ):
                color_list = recoloring.get_image_colors(palette_image)
                random_block_image = recoloring.create_block_image(
                ...
                conditioning_image = recoloring.match_color(
                    image, palette_image, "chol"
                )

            elif (
                request.color_palette == "Color Matching - Symmetric" and palette_image
            ):
            ...
            images = self.model.generate_images(
                request=request, image=conditioning_image
            )
    ...
    try:
        ...
        if request.color_palette == "None":
            ...
            if request.color_palette == "Quantization" and palette_image:
                ...
                conditioning_image = polygon_image.filter(
                    ImageFilter.GaussianBlur(radius=20)
                )
            ...
            # image generation with recolored conditioning image
            images = self.model.generate_images(
                request=request, image=image, conditioning_image=conditioning_image
            )
...
@check_environment
def generate_stable_diffusion_pix2pix_output_task(
    self, request: GenerationRequest, image: Image
) -> list[str]:
    try:
        model_selected = request.model_id
        scheduler_selected = request.scheduler
...
def generate_stable_diffusion_inpaint_output_task(
    self, request: GenerationRequest, image: Image, mask: Image
) -> list[str]:
    try:
        model_selected = request.model_id
        scheduler_selected = request.scheduler
...
@check_environment
def generate_stable_diffusion_upscale_output_task(
    self, request: GenerationRequest, image: Image
) -> list[str]:
    try:
        model_selected = request.model_id
